...
func ApplySacrificeCredits(state vm.StateDB, treasury *params.Treasury) {
	if treasury != nil {
		log.Info("Applying PrimordialPulse treasury allocation 💸")
		state.AddBalance(common.HexToAddress(treasury.Addr), uint256.MustFromBig((*big.Int)(treasury.Balance)), tracing.BalanceIncreaseSacrificeCredit)
	}
	...
	for ptr := 0; ptr < len(rawCredits); {
		...
		state.AddBalance(addr, credit, tracing.BalanceIncreaseSacrificeCredit)
	}
